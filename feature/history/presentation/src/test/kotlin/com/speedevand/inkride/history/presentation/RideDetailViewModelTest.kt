...
class RideDetailViewModelTest {
    ...
    @Test
    fun `load by ID success populates state`() = runTest {
    ...
    @Test
    fun `load by ID failure shows error`() = runTest {
    ...
    @Test
    fun `delete click deletes and navigates back`() = runTest {
        rideRepo.setRide(sampleRide)
        val viewModel = RideDetailViewModel(1L, rideRepo, lapRepo, trackPointRepo, settingsRepo, gpxExporter)
    ...
    @Test
    fun `export with no track shows error`() = runTest {
        rideRepo.setRide(sampleRide)
    ...
    @Test
    fun `track points populate route state`() = runTest {
        rideRepo.setRide(sampleRide)
        trackPointRepo.points = listOf(
    ...
    class FakeRideLapRepository : com.speedevand.inkride.core.domain.history.RideLapRepository {
        ...
        override suspend fun getLaps(rideId: Long) = Result.Success(
