...
class RideDetailViewModel(
    ...
    fun onAction(action: RideDetailAction) {
        when (action) {
            RideDetailAction.OnDeleteClick -> viewModelScope.launch {
                rideHistoryRepository.deleteById(rideId)
                _events.send(RideDetailEvent.NavigateBack)
            }
            RideDetailAction.OnBackClick -> viewModelScope.launch {
                _events.send(RideDetailEvent.NavigateBack)
            }
            RideDetailAction.OnExportGpxClick -> viewModelScope.launch {
                gpxExporter.export(rideId)
