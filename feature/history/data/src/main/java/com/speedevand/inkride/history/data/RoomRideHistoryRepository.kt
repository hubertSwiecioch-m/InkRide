...
class RoomRideHistoryRepository(
    ...
    override fun observeAll(): Flow<List<RideRecord>> = dao.observeAll().map { list ->
...
private fun RideHistoryEntity.toRideRecord() = RideRecord(
    id = id,
    ...
    bikeType = try { BikeType.valueOf(bikeType) } catch (e: Exception) { BikeType.ROAD }
)

private fun RideRecord.toEntity() = RideHistoryEntity(
    id = id,
    startTimestamp = startTimestamp,
    endTimestamp = endTimestamp,
    distanceKm = distanceKm,
